...
CDirectXSphere::CDirectXSphere() : m_bMouseDown(false)
{
}


CDirectXSphere::~CDirectXSphere()
{
}

// 큐브를 그리기 위한 준비 과정
bool CDirectXSphere::CreateChild()
{
...
void CDirectXSphere::OnMouseDown( HWND hWnd, WPARAM btnState, int x, int y )
{
	m_sttMousePos.x = x;
	m_sttMousePos.y = y;
	m_bMouseDown = true;
...
void CDirectXSphere::OnMouseUp( WPARAM btnState, int x, int y )
{
	ReleaseCapture();
	m_bMouseDown = false;
}

void CDirectXSphere::OnMouseMove( HWND hWnd, WPARAM btnState, int x, int y )
{
	if( m_bMouseDown )
	{
